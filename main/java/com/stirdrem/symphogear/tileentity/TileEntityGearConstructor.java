package com.stirdrem.symphogear.tileentity;
...
public class TileEntityGearConstructor extends TileEntity
{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.gear_constructor");
	}
	...
	public static boolean isBurning(TileEntityGearConstructor te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			GearConstructor.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = GearConstructorRecipes.getInstance().getSinteringResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = GearConstructorRecipes.getInstance().getConstructorResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
